      ...
      <div className="bg-slate-900 w-full max-w-6xl max-h-[92vh] rounded-[3rem] border border-slate-800 shadow-2xl overflow-hidden flex flex-col">
        ...
        <div className="p-8 bg-slate-900/50 border-b border-slate-800 flex flex-wrap gap-6 items-end">
          ...
          <div className="flex gap-2">
            <button onClick={() => handleBulkAction('select_filtered')} className="px-6 py-4 bg-indigo-600/10 hover:bg-indigo-600/20 text-indigo-400 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-indigo-600/30">Ativar</button>
            <button onClick={() => handleBulkAction('deselect_filtered')} className="px-6 py-4 bg-slate-800 hover:bg-slate-700 text-slate-400 rounded-2xl text-[10px] font-black uppercase tracking-widest border border-slate-700">Limpar</button>
          </div>
        </div>

        <div className="flex-1 overflow-y-auto p-8 grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-3 scrollbar-hide">
          {filteredFiles.map(file => (
            <div key={file.path} onClick={() => file.type === 'text_file' && toggleFileSelection(file.path)} className={`p-4 rounded-2xl border cursor-pointer transition-all flex items-center gap-3 ${file.selected ? 'bg-indigo-600/10 border-indigo-500' : 'bg-slate-800/20 border-slate-800'}`}>
              <div className={`w-4 h-4 rounded border flex-shrink-0 flex items-center justify-center ${file.selected ? 'bg-indigo-600 border-indigo-500' : 'bg-slate-950 border-slate-700'}`}>
        ...
        <div className="p-8 border-t border-slate-800 bg-slate-900/90 flex items-center justify-between">
          ...
          <button onClick={onClose} className="px-10 py-4 bg-indigo-600 text-white rounded-2xl font-black text-sm uppercase tracking-widest shadow-2xl">Confirmar Workspace</button>
        </div>
      </div>
    </div>
  );
